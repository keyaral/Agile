...
public class EadesSpringEmbedder {
	...
	public double MAGNETIC_STRENGTH = 60000.0;
	...
	public void step(double timeStep, int mouseX, int mouseY){
		...
		for (Node n : graph.nodes) {
			...
			double x1Dist = pos.getX();
			double x2Dist = width-pos.getX();
			double y1Dist = pos.getY();
			double y2Dist = height-pos.getY();
			tempForce = tempForce.add(new Vector2D(n.getCharge() * WALL_CHARGE / x1Dist / x1Dist * MAGNETIC_STRENGTH, 0));
			tempForce = tempForce.add(new Vector2D(-n.getCharge() * WALL_CHARGE / x2Dist / x2Dist * MAGNETIC_STRENGTH, 0));
			tempForce = tempForce.add(new Vector2D(0, n.getCharge() * WALL_CHARGE / y1Dist / y1Dist * MAGNETIC_STRENGTH));
			tempForce = tempForce.add(new Vector2D(0, -n.getCharge() * WALL_CHARGE / y2Dist / y2Dist * MAGNETIC_STRENGTH));
